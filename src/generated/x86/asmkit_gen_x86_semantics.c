...
#if defined(ASMKIT_GEN_X86_SEMANTICS_REQUIRE_NONEMPTY) && ASMKIT_GEN_X86_SEMANTICS_REQUIRE_NONEMPTY
typedef char asmkit_gen_x86_requires_nonempty[(ASMKIT_X86_RIP_REL + 1u) > 0u ? 1 : -1];
#endif

#include "asmkit_gen_x86_semantics_asmkit_x86_feature_infos.inc"

const asmkit_feature_info_t* asmkit_gen_x86_feature_info_by_index(uint32_t index)
{
    if (index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_feature_infos)) {
        return 0;
    }
    return &asmkit_x86_feature_infos[index];
}

const asmkit_feature_info_t* asmkit_gen_x86_feature_info(uint32_t id)
{
    return asmkit_gen_x86_feature_info_by_index(id);
}

uint32_t asmkit_gen_x86_feature_count(void)
{
    return (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_feature_infos);
}

#include "asmkit_gen_x86_semantics_asmkit_x86_predicate_infos.inc"

const asmkit_predicate_info_t* asmkit_gen_x86_predicate_info_by_index(uint32_t index)
{
    if (index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_predicate_infos)) {
        return 0;
    }
    return &asmkit_x86_predicate_infos[index];
}

const asmkit_predicate_info_t* asmkit_gen_x86_predicate_info(uint32_t id)
{
    return asmkit_gen_x86_predicate_info_by_index(id);
}

uint32_t asmkit_gen_x86_predicate_count(void)
{
    return (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_predicate_infos);
}

#include "asmkit_gen_x86_semantics_asmkit_x86_cpu_feature_ids.inc"

#include "asmkit_gen_x86_semantics_asmkit_x86_cpu_infos.inc"

const asmkit_cpu_info_t* asmkit_gen_x86_cpu_info_by_index(uint32_t index)
{
    if (index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_cpu_infos)) {
        return 0;
    }
    return &asmkit_x86_cpu_infos[index];
}

const asmkit_cpu_info_t* asmkit_gen_x86_cpu_info(uint32_t id)
{
    return asmkit_gen_x86_cpu_info_by_index(id);
}
...
const asmkit_feature_info_t* asmkit_gen_x86_cpu_feature_info(uint32_t id, uint32_t index)
{
    ...
    cpu = asmkit_gen_x86_cpu_info(id);
    if (cpu == 0 || index >= cpu->feature_count) {
        return 0;
    }
    if (cpu->feature_index + index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_cpu_feature_ids)) {
        return 0;
    }
    feature_id = asmkit_x86_cpu_feature_ids[cpu->feature_index + index];
    return asmkit_gen_x86_feature_info(feature_id);
}

uint32_t asmkit_gen_x86_cpu_count(void)
{
    return (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_cpu_infos);
}

#include "asmkit_gen_x86_semantics_asmkit_x86_register_infos.inc"

static const uint32_t asmkit_x86_register_info_index_by_id[] = {
...
{
    if (index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_register_infos)) {
        return 0;
    }
    return &asmkit_x86_register_infos[index];
}

const asmkit_register_info_t* asmkit_gen_x86_register_info(uint32_t id)
{
    uint32_t index;
    if (id >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_register_info_index_by_id)) {
        return 0;
    }
    index = asmkit_x86_register_info_index_by_id[id];
    if (index == 0u) {
        return 0;
    }
    return &asmkit_x86_register_infos[index - 1u];
}

uint32_t asmkit_gen_x86_register_count(void)
{
    return (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_register_infos);
}

#include "asmkit_gen_x86_semantics_asmkit_x86_operand_type_infos.inc"

static const uint32_t asmkit_x86_operand_type_info_index_by_id[] = {
    ...
    113u, 114u, 115u, 116u, 117u, 118u,
};

const asmkit_operand_type_info_t* asmkit_gen_x86_operand_type_info_by_index(uint32_t index)
{
    if (index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_operand_type_infos)) {
        return 0;
    }
    return &asmkit_x86_operand_type_infos[index];
}

const asmkit_operand_type_info_t* asmkit_gen_x86_operand_type_info(uint32_t id)
{
    uint32_t index;
    if (id >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_operand_type_info_index_by_id)) {
        return 0;
    }
    index = asmkit_x86_operand_type_info_index_by_id[id];
    if (index == 0u) {
        return 0;
    }
    return &asmkit_x86_operand_type_infos[index - 1u];
}

uint32_t asmkit_gen_x86_operand_type_count(void)
{
    return (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_operand_type_infos);
}

#include "asmkit_gen_x86_semantics_asmkit_x86_operand_infos.inc"

#include "asmkit_gen_x86_semantics_asmkit_x86_register_effect_infos.inc"

#include "asmkit_gen_x86_semantics_asmkit_x86_instruction_infos.inc"

#include "asmkit_gen_x86_semantics_asmkit_x86_instruction_info_index_by_id.inc"

const asmkit_instruction_info_t* asmkit_gen_x86_instruction_info_by_index(uint32_t index)
{
    if (index >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_instruction_infos)) {
        return 0;
    }
    return &asmkit_x86_instruction_infos[index];
}

const asmkit_instruction_info_t* asmkit_gen_x86_instruction_info(uint32_t id)
{
    uint32_t index;
    if (id >= (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_instruction_info_index_by_id)) {
        return 0;
    }
    index = asmkit_x86_instruction_info_index_by_id[id];
    if (index == 0u) {
        return 0;
    }
    return &asmkit_x86_instruction_infos[index - 1u];
}

uint32_t asmkit_gen_x86_instruction_count(void)
{
    return (uint32_t)ASMKIT_ARRAY_COUNT(asmkit_x86_instruction_infos);
}

const asmkit_operand_info_t* asmkit_gen_x86_instruction_operand_infos(uint32_t id, uint32_t* ASMKIT_RESTRICT_X86_SCRATCH out_count)
{
    const asmkit_instruction_info_t* info;
    if (out_count != 0) { *out_count = 0u; }
    info = asmkit_gen_x86_instruction_info(id);
    if (info == 0 || info->operand_info_count == 0u) {
        return 0;
    }
    if (out_count != 0) { *out_count = info->operand_info_count; }
    return &asmkit_x86_operand_infos[info->operand_info_index];
}
...
const asmkit_operand_info_t* asmkit_gen_x86_instruction_operand_info(uint32_t id, uint32_t operand_index)
{
    ...
    return &asmkit_x86_operand_infos[info->operand_info_index + operand_index];
}
...
const asmkit_instruction_register_effect_t* asmkit_gen_x86_instruction_register_effect(uint32_t id, uint32_t effect_index)
{
    ...
    return &asmkit_x86_register_effect_infos[info->register_effect_index + effect_index];
}
